...
import { ArrowLeft, Save, AlertCircle, CheckCircle, MapPin, FileText, Activity, Globe } from "lucide-react";
...
export default function SubmitPermit() {
  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      try {
        ...
        const approved = currentUser.role === "admin" || currentUser.accessApproved;
        if (!approved) {
          setError("Your account is pending manual approval, so you cannot submit permits yet.");
        }
      } catch (sessionError) {
        if (!active) return;
        setError(sessionError instanceof Error ? sessionError.message : "Could not verify your account.");
      } finally {
        if (active) {
          setIsLoading(false);
        }
  ...
  const canSubmit = Boolean(user && (user.role === "admin" || user.accessApproved));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!canSubmit) {
      setError("An approved account is required before you can submit permits.");
      return;
    }
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Globe className="absolute left-3 top-3 text-gray-500 w-4 h-4" />
                  <select
                    name="country"
            ...
            <div className="space-y-2">
              ...
              <textarea
                ...
                onChange={handleChange}
                className="w-full bg-white border border-slate-200 rounded-xl py-2.5 px-4 text-slate-900 focus:outline-none focus:border-emerald-500 transition-colors min-h-[110px]"
                placeholder="Add context, links, or public comment deadlines if available."
              />
            </div>
